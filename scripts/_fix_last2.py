...
DB = r"DeepPhilosophy/backend/philosophers_db.py"
...
SP = '你是哲学史教授。请为这位哲学家撰写1000-1200字的思想深度剖析。结构：生平背景2句，3个核心概念深入剖析，哲学史地位和影响，2-3部代表作。纯文本，不用markdown。直接输出正文。'
...
for name in ['柏拉图', '尼采']:
    ...
    d = json.dumps({"model": "deepseek-chat", "messages": [
...
# Verify
ast.parse(content)
print(f'Written: {len(content)} bytes. Valid Python. DONE!')
