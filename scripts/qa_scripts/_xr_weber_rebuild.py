# -*- coding: utf-8 -*-
"""西方哲学史(英汉对照) c39ded430a1e 按 epub ncx 重建（一次性）
epub: 合集&概述/西方哲学史（英汉对照）.epub  ncx 217 条
结构: 中文(修订版前言~索引) + 英文(Preface~Index) 双对照
映射: depth0 有子=part(编/PART)，无子=独立 chapter；depth1=chapter(篇)；depth2+=section(toc 标记)
用法: python _xr_weber_rebuild.py [--dry]
"""
import zipfile, re, json, os, sys, html as html_mod, shutil, bisect

EP = 'F:/philosophy/西方/合集&概述/西方哲学史（英汉对照）.epub'
SRC = "f:/program/Python/PhiAgent/backend/data/book_chapters/c39ded430a1e"
DST = "f:/program/Python/DeepPhilosophy/DeepPhilosophy/backend/data/book_chapters/c39ded430a1e"
...
print(f"ncx 条目: {len([n for n in (lambda acc: (acc.extend(tree), acc)[1])([])])}")
...
def anchor_order():
    # calibre 生成的 epub 中 body 哈希 id 会在多个文件复用（如 7K4G0-...
    # 出现在 目录页 part0002 + split_000~004 的 <body> 上）
    # → 必须按文件分组，aid 只在 src 指定的文件内定位
    d = {}
...
for node in tree:
    ...
    for c in chs:
        ...
        # 终点: flat 中 c 之后的下一个 depth<=1 条目（树序）
...
for idx in range(0, len(files), 5):
    ch = files[idx]
    first = ch['content'][0]['value'][:40] if ch['content'] else '(空)'
    print(f"  [{idx}] {ch['title'][:38]} {len(ch['content'])}块 | {first}…")
...
meta = {
    "bookId": old_meta.get("bookId") or "c39ded430a1e",
    "title": old_meta.get("title") or "西方哲学史（英汉对照）",
    "author": old_meta.get("author") or "梯利",
    "toc": toc,
