...
n_in = n_nopdf = n_ocr = 0
for bid in A2:
    rel = rel_by_bid.get(bid)
    if not rel:
        print(f"✗ {bid}: 不在 ckpt books"); continue
    n_in += 1
    safe = rel.replace("/", "_").replace("\\", "_")
    has_ocr = bool(ocr.get(safe))
    pdf = os.path.join("F:/philosophy", rel.replace("/", os.sep))
    ok_pdf = os.path.exists(pdf)
    if has_ocr: n_ocr += 1
    else:
        n_nopdf += (not ok_pdf)
        print(f"{'✗' if not ok_pdf else '…'} {bid}: ocr={'有' if has_ocr else '无'} pdf={'有' if ok_pdf else '无'} {rel[:70]}")
print(f"共{len(A2)} 在ckpt:{n_in} 有ocr:{n_ocr} 无ocr且pdf不存在:{n_nopdf}")
