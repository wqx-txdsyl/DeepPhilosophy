# -*- coding: utf-8 -*-
"""《与神对话(全5卷)》重建 v4 — 在 v3 基础上修复重复插入
v3 遗留: 旧库段与黄金流切分不同(旧库把黄金流2段合成1段)时, 边界歧义把
"黄金流已有整段"也拉进插入列表 → 重复插入 27 字(3 段: "我来看看能否总结出
你在第三章中谈到的要点。"21字 + "是："×2)。黄金独有 0 ✓ 已证无丢失。
v4: 段收集时 `dw(t) in gold_dw` 整段已在黄金流 → 丢弃(不插入)。
预期: 插入 103 段 ~3493 字, 新库独有 = 0
"""
...
for op, i1, i2, j1, j2 in ops:
    ...
    for t in segs:
        ...
        if is_junk(t):
            ...
            dup_list.append(t)      # 黄金流已有 → 边界歧义拉入 → 丢弃
        else:
