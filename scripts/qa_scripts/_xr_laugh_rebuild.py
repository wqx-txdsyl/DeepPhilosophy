# -*- coding: utf-8 -*-
"""别笑，我是正经哲学书 a1448e164acc 按 epub ncx 重建（一次性，锚点#s无效→标题定位）
epub: 富增章成/别笑，我是正经哲学书.epub  ncx 68 条
映射: d0 有子=part（Chapter 1-4），d0 无子=chapter（前言/参考文献/结语），d1=chapter（问题篇）
      d1 锚点无效(#s)时按标题文本定位；跳过 封面
用法: python _xr_utopia_rebuild.py [--dry]
"""
import zipfile, re, json, os, sys, html as html_mod, shutil, bisect

EP = 'F:/philosophy/东方/富增章成/别笑，我是正经哲学书.epub'
SRC = "f:/program/Python/PhiAgent/backend/data/book_chapters/a1448e164acc"
DST = "f:/program/Python/DeepPhilosophy/DeepPhilosophy/backend/data/book_chapters/a1448e164acc"
SKIP = {"封面", "版权", "版权页", "目录", "总目录"}
...
def find_by_label(f, label):
    """在文件 f 的块中找 norm 匹配 label 的第一个块（ncx 锚点无效时的标题定位）"""
    bl = blocks_by_file[f]
    nl = norm(label)
    for kbi, (off, kind, text) in enumerate(bl):
        if norm(text) == nl:
            return kbi
    return None

def seg_text(src, label=None):
    ...
    if f in htmls:
        if aid and aid in ANCH.get(f, {}):
            return [ANCH[f][aid]]
        if aid and label:
            # ncx 锚点无效（如全为 #s）→ 按标题文本定位
            bi = find_by_label(f, label)
            if bi is not None:
                return [(htmls.index(f), bi)]
        if aid:
            print(f"  !! 锚点缺失: {src}")
...
for node in tree:
    ...
    for c in chs:
        ...
        for f2 in flat[idx_flat + 1:]:
            if f2['level'] <= 1:
                s2 = seg_text(f2['src'], f2.get('label'))
                if s2:
                    end_fi, end_bi = s2[0]
                    break
        if end_fi is None:
