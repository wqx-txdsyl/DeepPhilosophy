# -*- coding: utf-8 -*-
"""叔本华哲学经典（套装共5册）d0c5ade4fcbd 重建（一次性）
epub 源: 叔本华哲学经典（套装共5册）ncx 221 条完整目录
方案:
- ncx depth0 = 册 → part；depth1 = chapter；depth2 = section
...
EP = 'F:/philosophy/西方/阿图尔·叔本华/叔本华哲学经典（套装共5册）.epub'
SRC = "f:/program/Python/PhiAgent/backend/data/book_chapters/d0c5ade4fcbd"
DST = "f:/program/Python/DeepPhilosophy/DeepPhilosophy/backend/data/book_chapters/d0c5ade4fcbd"
SKIP = {"总目录", "封面", "版权", "版权页", "目录"}
REPEAT_DROP = {"叔本华年谱", "叔本华的生平及哲学"}  # 每册附录重复，只留首次
...
for b in books:
    ...
    for ci, c in enumerate(chs):
        ...
        if nl in SKIP:
            continue
        # 每册附录重复（年谱/生平及哲学）只保留首次出现
        if nl in REPEAT_DROP:
            if nl in seen_repeat:
                print(f"  -- 重复跳过: {c['label']}")
                continue
            seen_repeat[nl] = True
        # 起点
        seq = seg_text(c['src'])
...
meta = {
    "bookId": old_meta.get("bookId") or "d0c5ade4fcbd",
    "title": old_meta.get("title") or "叔本华哲学经典（套装共5册）",
