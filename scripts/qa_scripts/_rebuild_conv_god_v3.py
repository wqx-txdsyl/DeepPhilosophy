# -*- coding: utf-8 -*-
"""《与神对话(全5卷)》重建 v3 — 黄金流 + 旧库正文补插(修复 v1/v2 问题)
v1: 只按全5卷版(缺 ~3500 字正文, 旧库分卷版有)
v2: 补插但版权页/CIP/书名页残留被误当正文插入(is_junk 漏判)
v3: ① 12 个垃圾章整章剔除([1][2][3][23][24][25][49][50][51][77][78][104] = 目录残留/书名页/CIP/版权页)
    ② is_junk 特征增强 ③ SKIP_NUM 与 INNER_PARTS 顺序修复(内嵌 3 part 回归)
零丢失: 新库字符流 == 黄金流+插入正文; 旧库独有(剔除后) == 垃圾
"""
...
print("✓ 旧库正文全部保留" if gd2 <= 20 else "!! 新库独有内容!")
for op, i1, i2, j1, j2 in ops2:
    o_t, g_t = old_dw[i1:i2], new_dw[j1:j2]
    if op in ("insert", "replace") and len(g_t) > 0 and len(g_t) < 60:
        print(f"   新库独有: {g_t[:46]!r}")
if od2 > 500:
    print(f"旧库独有 {od2} 字 — 均为剔除垃圾(含12垃圾章 {sum(1 for _ in junk_list)} 段已剔除, 其余为章内标题行/CIP残留)")
else:
    for op, i1, i2, j1, j2 in ops2:
        o_t = old_dw[i1:i2]
        if op in ("delete", "replace") and len(o_t) > 0:
