# -*- coding: utf-8 -*-
"""乌合之众 e58770681143 按 epub ncx 重建（一次性，卷/章同级需包裹）
epub: 古斯塔夫·勒庞/乌合之众：大众心理研究.epub  ncx 152 条（全平）
映射: 自定义——d0 匹配"第X卷"=part，匹配"第X章/结束章"=chapter，d1=section
      跳过 版权
用法: python _xr_utopia_rebuild.py [--dry]
"""
import zipfile, re, json, os, sys, html as html_mod, shutil, bisect

EP = 'F:/philosophy/西方/古斯塔夫·勒庞/乌合之众：大众心理研究.epub'
SRC = "f:/program/Python/PhiAgent/backend/data/book_chapters/e58770681143"
DST = "f:/program/Python/DeepPhilosophy/DeepPhilosophy/backend/data/book_chapters/e58770681143"
...
# ────────── 3. 组装（卷/章同级 → 卷包裹） ──────────
R_VOL = re.compile(r'^第[一二三四五六七八九十百]+卷')
R_CH = re.compile(r'^第[一二三四五六七八九十百]+章|^结束章')
toc = []
files = {}
ch_index = 0
warns = []
...
for node in tree:  # 全部 d0 平铺
    ...
    if R_VOL.match(node['label']):
        # 卷标题页无正文（书名+卷名），只做 part 标记
        toc.append({"type": "part", "title": node['label'], "level": 0, "index": ch_index})
        continue
    if not R_CH.match(node['label']):
        warns.append(f"!! 未知条目: {node['label']}")
        continue
    ...
    for f2 in flat[idx_flat + 1:]:
        if f2['level'] == 0 and (R_CH.match(f2['label']) or R_VOL.match(f2['label'])):
            s2 = seg_text(f2['src'])
            if s2:
                end_fi, end_bi = s2[0]
                break
