# -*- coding: utf-8 -*-
"""企鹅哲学史 4e1a78c6f009 按 epub ncx 重建（一次性）
epub: A.C.格雷林/企鹅哲学史.epub  ncx 90 条
映射: d0 有子=part（五个部分），d0 无子=chapter（前言/导论等），d1=chapter，d2=section
      跳过 书名页/版权页/品牌页/正文目录
用法: python _xr_utopia_rebuild.py [--dry]
"""
import zipfile, re, json, os, sys, html as html_mod, shutil, bisect

EP = 'F:/philosophy/西方/A.C.格雷林/企鹅哲学史 -A.C.格雷林 .epub'
SRC = "f:/program/Python/PhiAgent/backend/data/book_chapters/4e1a78c6f009"
DST = "f:/program/Python/DeepPhilosophy/DeepPhilosophy/backend/data/book_chapters/4e1a78c6f009"
SKIP = {"书名页", "版权页", "品牌页", "正文目录", "版权", "目录", "总目录", "封面", "索引", "哲学家年表"}
...
htmls = sorted((n for n in z.namelist() if n.endswith('.html')),
...
for node in tree:
    ...
    for c in chs:
        ...
        n0 = norm(blocks[0]["value"]).replace('—', '-').replace('–', '-') if blocks else ''
        nl0 = norm(c['label']).replace('—', '-').replace('–', '-')
        if blocks and (n0 == nl0 or (n0.startswith(nl0) and len(blocks[0]["value"]) < 80)):
            blocks = blocks[1:]
        if not blocks:
...
meta = {
    "bookId": old_meta.get("bookId") or "4e1a78c6f009",
    "title": old_meta.get("title") or "企鹅哲学史",
    "author": old_meta.get("author") or "A.C.格雷林",
    "toc": toc,
