# -*- coding: utf-8 -*-
"""鬼谷子 1f6fe151032b 修复（一次性，ncx 锚点切分，拆上中下三卷 + 去前缀）
epub: 鬼谷子.epub  ncx 24 条（关于本书/目录/CoverPage 垃圾 + 21 篇平铺）
旧数据 22 章 = 关于本书 + 21 篇（"鬼谷子·/本经阴符七术·/持枢·/中经·"前缀混标题），无卷级。
结构:
  SKIP: 关于本书/目录/CoverPage
  part(0) × 3: 上卷（鬼谷子·12 篇）/ 中卷（本经阴符七术·7 篇）/ 下卷（持枢/中经）
  chapter × 21: 标题去前缀（捭阖/反应/…盛神法五龙…/持枢/中经）
用法: python _xr_guiguzi_rebuild.py [--dry]
"""
import zipfile, re, json, os, sys, html as html_mod, shutil, bisect

EP = 'F:/philosophy/东方/鬼谷子/鬼谷子.epub'
SRC = "f:/program/Python/PhiAgent/backend/data/book_chapters/1f6fe151032b"
DST = "f:/program/Python/DeepPhilosophy/DeepPhilosophy/backend/data/book_chapters/1f6fe151032b"
SKIP = {"关于本书", "目录", "CoverPage"}
PREFIX_RE = re.compile(r'^(鬼谷子|本经阴符七术|持枢|中经)·')
TITLE_RE = re.compile(r'^(鬼谷子|本经阴符七术|持枢|中经)·?(全篇)?$')
...
# 卷划分：鬼谷子·12 篇 → 上卷；本经阴符七术·7 篇 → 中卷；其余（持枢/中经）→ 下卷
VOLS = [("上卷", "鬼谷子"), ("中卷", "本经阴符七术"), ("下卷", "持枢")]
...
for i, node in enumerate(flat):
    ...
    if not blocks:
        ...
        continue
    title = '中经' if node['label'] == '中经·全篇' else PREFIX_RE.sub('', node['label'])
...
for idx, ch in files.items():
    ...
    total_chars += nc
print(f"\n总: {len(files)} 章, {total_chars} 字符（旧 10,550）")
for tt in toc:
    if tt['type'] == 'part':
        print(("  " * tt['level']) + f"[{tt['level']}] {tt['title']} → index {tt['index']}")
print("标题:", [files[i]['title'] for i in range(len(files))])
...
meta = {
    "bookId": old_meta.get("bookId") or "1f6fe151032b",
    "title": old_meta.get("title") or "鬼谷子",
    "author": old_meta.get("author") or "鬼谷子",
    "toc": toc,
