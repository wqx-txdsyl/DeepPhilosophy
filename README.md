...
> 横跨五千年的人类思想史长卷 —— 409 部哲学经典 · 737 位哲学家 · 111 个哲学流派 · 一个能对话、思辨、创作的哲学智能体

**单仓库双应用**（2026-08-14 合并）：`app/` 是开放的哲学知识平台（阅读/探索/问答，生产部署在 [deepphilosophy.top](https://deepphilosophy.top)）；`agent-app/` + `backend/` 是哲学智能体平台（LangGraph 驱动的「深哲」与由 AIAuthor 六库数字人格构建的「尼采」）；`backend/tools/` 承载 409 部原典的数字化工序（导入/OCR/章节重建/向量化/同步）。

---

## 📖 平台功能（deepphilosophy.top）

- **409 部哲学经典在线阅读**（数量持续增长，以 `app/public/books.json` 为准，2026-08-18 核对）：结构化章节 + 书内插图，章节走 OSS/jsDelivr 双轨 CDN（秒开级加载），阅读中可 AI 批注、与书对话
...
| 类别 | 数量 |
|---|---|
| 经典著作 | 409 部（以 `app/public/books.json` 为准，2026-08-18 核对） |
| 哲学家 | 737 位 |
| 哲学流派 | 111 个 |
| 章节 | 12210 个（312 本书） |
