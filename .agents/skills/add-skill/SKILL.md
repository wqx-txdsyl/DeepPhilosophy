...
- [ ] 步骤 2：文件生成 — 按模板写入 `.Codex/skills/ARG_NAME/SKILL.md`（目录 + SKILL.md 结构）
...
```bash
python -c "
import os; p='.Codex/skills/ARG_NAME/SKILL.md'
...
```bash
python -c "
with open('.Codex/skills/ARG_NAME/SKILL.md',encoding='utf-8') as f: c=f.read()
checks=[]
...
- **门禁验证（Check）**：`grep -r "ARG_NAME" .Codex/skills/ | wc -l` 确认引用数 >= 1。
...
- 产物：`.Codex/skills/ARG_NAME/SKILL.md`
