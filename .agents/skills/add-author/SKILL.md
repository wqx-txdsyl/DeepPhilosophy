...
## 前置依赖
- `scripts/fetch_philosopher_img.py` `scripts/gen_portrait.py` `scripts/_lib.py`（DP 仓库）
- `PhiAgent/backend/tools/build_philosopher_network.py`（**在 PhiAgent 仓库**，DP 侧不重复维护，2026-08-11 分家后）
...
### 6. 更新星丛网络
```bash
# 在 PhiAgent 仓库执行（DP 侧无此脚本）
cd F:/program/Python/PhiAgent/backend/tools && python build_philosopher_network.py
```

### 7. 评分排序插入 books.json
确保 philosophers.json 按 rank 降序排列

### 8. 推送（精确 pathspec，禁用 git add -A）
```bash
git add app/public/philosophers.json backend/data/philosophers.json app/public/philosopher/
git commit -m "feat: 添加哲学家 ARG_NAME" && git push
```
