        ...
        if rootfile and rootfile in names:
            ...
            for it in opf.find_all('item'):
                ...
                if iid and href:
                    ...
                    full=posixpath.normpath(full)  # 处理 ../ 相对路径（如 toc.ncx）
                    items[iid]=full
            ...
            for it in opf.find_all('item'):
                if it.get('media-type')=='application/x-dtbncx+xml':
                    ...
                    ncx_path=posixpath.normpath(ncx_path)  # 处理 ../ 相对路径
        ...
        for ci, mc in enumerate(merged_chapters):
            ...
            for si in mc['spine_range']:
                ...
                if f is None:
                    continue
                if anchor:
                    # 锚点可能在 body 自身（每章一文件的 epub, 如世界哲学简史）——find 不含自身, 需单独检查
                    _body = f['soup']
                    el = _body if _body.get("id") == anchor else _body.find(id=anchor)
                    if el is not None:
