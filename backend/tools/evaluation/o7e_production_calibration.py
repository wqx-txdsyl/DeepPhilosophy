...
def run_case_production(case, mk_normal, mk_repair):
    ...
    for e in evs:
        if e.get("type") == "tool" and e.get("name") in (
            ...
            # V5-F2-R1.1 §2/§3: 原调用即时快照（engine 在 result_full 剥离与
            # cache 晋升前生成, 绑定 tool_call_id）; 事后重放/按 SID 反查已删除
            # （POST_RUN_REPLAY=false; CACHE_DEPENDENCE=false）
            scholarly_calls_detail.append({"tool": e.get("name"),
                                           "tool_call_id": e.get("tool_call_id"),
                                           "args": e.get("args"),
                                           "scholarly_trace": e.get("scholarly_trace")})
    ...
    record = {"case_id": case["case_id"],
              ...
              # V8-F2-R1 §4: 语义转移六项计数（来自 case_result 聚合, additive）
              "SEMANTIC_TRANSITION_COUNTS": tel.get("SEMANTIC_TRANSITION_COUNTS"),
              # V9 §glue: V8-F2 closure regression telemetry 透传（additive,
              # 不改 measurement semantics; 冻结于 V9 formal run 之前）
              "V8F2_TELEMETRY": done.get("v8f2_telemetry"),
