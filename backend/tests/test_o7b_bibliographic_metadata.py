...
RP1_CODE_SHA = "7ee6d2b62"  # routes 冻结基线（O7-C §59 授权改动落地 commit）
...
def test_t17_production_frozen_rp2():
    r = subprocess.run(["git", "diff", "--quiet", "7ee6d2b62", "--", "backend/routes"],
                       cwd=ROOT, capture_output=True)
    assert r.returncode == 0, "routes 相对 O7-C 授权基线 7ee6d2b62 有未授权改动"
