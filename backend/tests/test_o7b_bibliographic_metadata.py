...
RP1_CODE_SHA = "76c453190"  # routes 冻结基线（PF-RP4A §B scholarly 工具描述授权点）
...
# ══ R17 — 生产冻结 ═════════════════════════════════════════
HARD_FROZEN = ("backend/final_validator.py",
...
def test_t17_production_frozen_rp2():
    r = subprocess.run(["git", "diff", "--quiet", "76c453190", "HEAD", "--", "backend/routes"],
                       cwd=ROOT, capture_output=True)
    assert r.returncode == 0, "routes 相对 PF-RP4A §B 授权基线 76c453190 有未授权改动"
