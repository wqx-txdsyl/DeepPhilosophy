...
│  │  ├─ backend/
│  │  │  └─ data/
│  │  │     └─ book_chapters/  # 312 本书 × 12210 个章节 json (按 bid 分目录, 顶层 dict 禁 list)
│  │  ├─ book_detail/  # 402 个 detail json (三处同步规则: PHA/DP/app public)
│  │  ├─ covers/  # 前端静态资源 (402 文件)
│  │  ├─ gene/
│  │  │  ├─ region/
...
│  │  ├─ .nojekyll  (0 B)
│  │  ├─ books.json  (562,513 B)
│  │  ├─ covers.json  (21,729 B)
│  │  ├─ favicon.png  (11,953 B)
│  │  ├─ favicon.svg  (9,522 B)
│  │  ├─ icons.svg  (5,031 B)
│  │  ├─ philosopher_network.json  (394,531 B)
│  │  ├─ philosophers.json  (2,806,731 B)
│  │  ├─ robots.txt  (71 B)
│  │  └─ sitemap.xml  (1,074 B)
│  ├─ src/
│  │  ├─ assets/  — 前端打包资源
│  │  │  └─ books.json  (538,580 B)
│  │  ├─ components/  — 前端组件
│  │  │  ├─ school/  — 学派详情组件
...
│  │  │  │  └─ tokens.js  (735 B)
│  │  │  ├─ AvatarUpload.jsx  (6,558 B)  — 头像上传
│  │  │  ├─ ChapterReader.jsx  (31,919 B)  — 章节阅读器
...
│  │  │  └─ WorldMap.jsx  (11,307 B)  — 世界地图（思想地理）
│  │  ├─ contexts/  — React 上下文
...
│  │  │  ├─ BookDetailPage.jsx  (9,725 B)  — 书籍详情
│  │  │  ├─ BooksPage.jsx  (10,620 B)  — 书库列表
│  │  │  ├─ DeveloperPage.jsx  (5,362 B)  — 开发者后台（访问统计）
│  │  │  ├─ EasternPhilosophiesPage.jsx  (8,005 B)  — 东方哲学
│  │  │  ├─ GamesPage.jsx  (2,167 B)  — 小游戏
│  │  │  ├─ GenealogyPage.jsx  (28,703 B)  — 思想谱系图
│  │  │  ├─ HomePage.css  (9,679 B)
│  │  │  ├─ HomePage.jsx  (30,135 B)  — 首页
...
│  │  │  ├─ ProfilePage.jsx  (19,725 B)  — 个人中心
│  │  │  ├─ QAPage.jsx  (16,242 B)  — AI 问答（流式 + 自配 key 直连）
│  │  │  ├─ ReaderPage.jsx  (24,017 B)  — 阅读器（PDF/章节 + 批注/书聊）
│  │  │  ├─ SchoolDetailPage.jsx  (35,070 B)  — 学派详情
│  │  │  ├─ SettingsPage.jsx  (6,580 B)  — 设置
│  │  │  ├─ TermsPage.jsx  (1,730 B)  — 服务条款
│  │  │  ├─ WesternPhilosophiesPage.jsx  (9,466 B)  — 西方哲学
│  │  │  └─ WorldPhilosophiesPage.jsx  (14,166 B)  — 世界哲学
│  │  ├─ utils/  — 前端工具
│  │  │  ├─ api.js  (701 B)  — API 封装（生产同源/兜底链）
│  │  │  └─ seo.js  (1,280 B)  — SEO 工具
│  │  ├─ App.css  (27,095 B)
│  │  ├─ App.jsx  (9,762 B)  — 应用入口/路由
│  │  ├─ data.js  (2,296 B)  — 前端全局数据
│  │  ├─ index.css  (1,070 B)
│  │  └─ main.jsx  (211 B)  — 入口挂载
│  ├─ .env  (286 B)
...
│  │  ├─ __init__.py  (0 B)
│  │  ├─ admin_stats.json  (5,503 B)
│  │  ├─ book_rankings.json  (30,302 B)
│  │  ├─ book_summaries.json  (1,064,842 B)
│  │  ├─ books_catalog.json  (537,618 B)
...
│  │  └─ users.db  (90,112 B)
│  ├─ models/  — Pydantic 模型
│  │  └─ __init__.py  (1,115 B)  — 共享 Pydantic 请求/响应模型
│  ├─ modules/  — 核心功能模块
│  │  ├─ __init__.py  (48 B)
...
│  ├─ __init__.py  (25 B)
│  ├─ admin.py  (5,176 B)  — 开发者管理后台（访问统计 + 用户管理）
│  ├─ auth.py  (26,271 B)  — 用户认证（SQLite + OSS + GitHub Release 备份）
│  ├─ config.py  (4,500 B)  — 云端部署配置（全部路径走环境变量）
│  ├─ db.py  (2,243 B)  — 哲学家信息库（JSON 加载 O(1) 查找）
│  ├─ fix_bios.py  (2,418 B)  — 批量修复 <1000 字的哲学家 bio
│  └─ main.py  (90,018 B)  — 云端 API 服务器（书籍/文件/RAG/作者/用户/历史）
...
│  │  │  ├─ index.js  (20,576 B)  — api worker：业务端点 + JWT + SSE 透传
...
│  └─ auth/
│     ├─ src/  — auth worker 源码
│     │  └─ index.js  (5,602 B)  — auth worker：登录/注册/JWT
│     ├─ .dev.vars  (39 B)
