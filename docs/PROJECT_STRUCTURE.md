...
│  │  ├─ tag_normalization.json  (11,647 B)
│  │  └─ users.db  (90,112 B)
...
│  │  ├─ llm_client.py  (3,090 B)  — DeepSeek API 客户端（OpenAI 兼容）
...
│  │  └─ 分章标准规范.md  (7,223 B)
│  ├─ __init__.py  (25 B)
│  ├─ admin.py  (5,786 B)  — 开发者管理后台（访问统计 + 用户管理）
│  ├─ agents.py  (19,501 B)  — 智能体注册表（智能体广场）
│  ├─ auth.py  (30,567 B)  — 用户认证（SQLite dev / D1 prod; 整库云备份已禁用 2026-08-14）
│  ├─ config.py  (4,536 B)  — 云端部署配置（全部路径走环境变量）
│  ├─ db.py  (2,243 B)  — 哲学家信息库（JSON 加载 O(1) 查找）
│  ├─ drawio_convert.py  (4,977 B)  — mermaid → draw.io XML 转换器
│  ├─ engine_langgraph.py  (47,009 B)  — LangGraph 引擎（PhiAgent v2 流式编排）
...
│  │  │  ├─ index.js  (23,420 B)  — api worker：业务端点 + JWT + SSE 透传
...
│  └─ auth/
│     ├─ src/  — auth worker 源码
│     │  └─ index.js  (8,832 B)  — auth worker：登录/注册/JWT
│     ├─ .dev.vars  (39 B)
